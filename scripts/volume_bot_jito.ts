...
const TRADE_AMOUNT_SOL = 0.01; // SOL to buy with
...
// Jito Block Engine
const JITO_BLOCK_ENGINE_URL = "https://amsterdam.mainnet.block-engine.jito.wtf/api/v1/bundles";
...
async function run() {
    ...
    while (true) {
        try {
            ...
            // 3. Sell Quote (Token -> SOL)
            // Use the expected output from buy as input for sell
            ...
            tipTx.sign([wallet]);
            const tipTxBase64 = bs58.encode(tipTx.serialize());

            // 6. Deserialize and Sign Swaps
            // Jupiter returns signed or unsigned? Usually unsigned if we didn't pass signature.
            // Actually we need to sign them.
