...
*   Generates a unique "Magic Transaction" instruction for you.
*   Provides the **Community Address** you must send tokens to.
...
*   Shows the Minimum Balance needed.
*   Shows any Whale Thresholds or Banned Words.
...
*   *Note: You must set the group's Invite Link to "Request Admin Approval" for gating to work!*

#### `/create_community`
**Usage:** `/create_community` (DM Only)
**Description:** Launches the **Community Creation Wizard**.
*   The bot will interview you to set up new rules (Token Mint, Min Balance, Whale %).
*   It generates a **New Keypair** and a ready-to-broadcast instruction.
*   Use this to spin up a new token-gated community from scratch.

### 📊 Monitoring & Moderation

#### `/list_balances`
**Usage:** `/list_balances`
**Description:** Scans the group for verified members and displays a **Leaderboard**.
*   Shows the Top 50 holders in the chat.
*   Helpful for finding whales or checking community strength.

#### `/check_unverified`
**Usage:** `/check_unverified`
**Description:** Scans the group for members who have **slippped through**.
*   Lists any user in the chat who does *not* have a verified wallet in the bot's database.
*   Use this to clean up your group or find people who bypassed the join request.

#### `/check_user` (Reply)
**Usage:** Reply to a user's message with `/check_user`
**Description:** Spies on a specific user's status.
...
*   **Wait:** Solana can take 30-60 seconds to confirm. Wait a minute and try `/i_sent_it` again.
